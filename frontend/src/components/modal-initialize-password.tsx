...
export default function ModalInitializePassword({
  isOpen,
  setIsOpen,
  token,
}: {
  ...
  const [initializePassword] = useInitializePasswordMutation();
  const router = useRouter();

  const notify = () =>
    toast.success(
      "Votre mot de passe a bien été enregistré. Vous pouvez maintenant vous connecter."
    );

  const handleSubmitInitializePassword = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    setError("");
    e.preventDefault();
    if (!token) return setError("lien de d'initialisation invalide");
    ...
    try {
      await initializePasswordSchema.validate(formJSON, { abortEarly: false });
      await initializePassword({
        variables: { data: formJSON, passwordToken: token },
      });
  ...
  const initializePasswordSchema = Yup.object().shape({
    password: Yup.string()
            ...
            <div className="relative w-auto my-6 mx-2 md:mx-4 lg:mx-auto max-w-3xl">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative px-8 py-3 flex-auto">
                  ...
                  <form
                    onSubmit={handleSubmitInitializePassword}
